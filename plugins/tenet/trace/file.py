...
try:
    ...
    from tenet.trace.arch import ArchAMD64, ArchX86, ArchAARCH64
...
#
    ...
    from arch import ArchAMD64, ArchX86, ArchAARCH64
...
class TraceFile(object):
    ...
    def _select_arch(self, magic):
        ...
        if ArchAMD64.MAGIC == magic:
            self.arch = ArchAMD64()
        if ArchAARCH64.MAGIC == magic:
            self.arch = ArchAARCH64()
        else:
            self.arch = ArchX86()
...
class TraceSegment(object):
    ...
    def get_reg_info(self, idx, reg_names):
        ...
        for i, mask_id in enumerate(search_masks):
            ...
            for reg_name in found_names:
                found_registers[reg_name] = (
                    registers[reg_name],
                    (self.base_idx + (start_idx - i)),
                )
    ...
    def get_mem_data(self, mem_id, set_id, data_mask):
        ...
        offset = offsets[
            mem_id
        ]  # sum([number_of_bits_set(mask) for mask in masks[:mem_id]])
    ...
    def _process_lines(self, lines):
        ...
        # TODO: pretty gross, but let's just wrap it to make these issues more apparents
            ...
            pmsg(
                f"LINE PARSE FAILED, line ~{self.base_idx+relative_idx:,}, contents '{line}'"
            )
            pmsg(str(e))
    ...
    def _process_mem_entry(self, address, data, access_type, relative_idx):
        ...
        byte = 0
        for mapped_address, access_mask, access_data in self._map_mem_access(
            address, data
        ):
    ...
    def _map_mem_access(self, address, data):
        ...
        while remaining_mask:
            ...
            mapped_address = self.trace.mem_map.setdefault(
                aligned_address, len(self.trace.mem_map)
            )
    ...
    def _pack_registers(self, registers, relative_idx):
        ...
        value_pairs = sorted(
            [
                (self.arch.REGISTERS.index(name), value)
