...
class TenetContext(object):
    ...
    def __init__(self, core, db):
        ...
        print(disassembler, disassembler[self], DisassemblerContextAPI)
        self.core = core
        self.db = db

        # select a trace arch based on the binary the disassmbler has loaded
        arch = disassembler[self].get_processor_type()
        if arch == AARCH64:
    ...
    def load_trace(self, filepath):
        ...
        if self.reader.analysis.slide != None:
            ...
            pmsg(
                "   |  if you know it, and reload the trace. Otherwise, it is possible"
            )
